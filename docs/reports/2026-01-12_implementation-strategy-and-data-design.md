...
#### パターン3: Auto Increment（非推奨）

```typescript
// ❌ 非推奨：競合が発生しやすい
async function getNextId() {
  const kv = await getKv()
  const counter = await kv.get(['counters', 'users',],)
  const nextId = (counter.value as number || 0) + 1
...
const userId = await getNextId() // 1, 2, 3, ...
await kv.set(['users', userId,], user,)
```
...
const id = crypto.randomUUID()
await kv.set(['events', timestamp, id,], event,)

// これで時系列順に取得できる
const events = kv.list({ prefix: ['events',], },) // 自動的に時系列順
```
